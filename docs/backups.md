...
Identity schema version 3 contains profiles, relationships, explicit
shared-context access, temporary interaction sessions, and bounded agent runs,
steps, and events. Restore validation checks these tables before replacing the
live database. Agent goals, approved write arguments, and final results can be
private. Protect this backup with the same care as memory and device
credentials.
